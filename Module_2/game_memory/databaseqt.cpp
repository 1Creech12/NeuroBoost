...
bool PlayerDatabase::connect(const QString &dbPath)
{
    ...
    if (!query.next()) {
        ...
        qDebug() << "[PlayerDatabase] База данных создана заново:" << dbPath;
        if (!createTable()) {
            emit errorOccurred("Не удалось создать таблицу users");
            return false;
        }
    } else {
        qDebug() << "[PlayerDatabase] Подключено к существующей БД:" << dbPath;
    }
